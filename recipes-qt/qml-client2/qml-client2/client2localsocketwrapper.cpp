...
Client2LocalSocketWrapper::Client2LocalSocketWrapper(QObject *parent)
...
void Client2LocalSocketWrapper::connectToServer(const QString &path)
{
    ...
    m_stopPolling = false;
    m_pollingThread = std::thread(&Client2LocalSocketWrapper::startPolling, this);
}

void Client2LocalSocketWrapper::startPolling()
{
...
void Client2LocalSocketWrapper::handleReadyRead()
{
    char buffer[1024];
...
void Client2LocalSocketWrapper::sendData(const QString &data)
{
    if (m_socket < 0) {
...
void Client2LocalSocketWrapper::handleError(const std::string &errorString)
{
